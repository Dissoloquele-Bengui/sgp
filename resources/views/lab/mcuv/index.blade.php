...
@section('titulo', 'Experências(MCUV)')
@section('conteudo')
    <div class="row mb-3">
        <div class="col">
                ...
                <div class="row gx-0 flex-between-center">
                    <div class="col-sm-auto d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <h4 class="text-primary fw-bold mb-0">Movimento Circular Uniformemente Variado (MCUV) <span
            ...
            <div class="card h-100 ">
                <div class="card-header " data-bs-theme="light">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col">
                            <h4 class="text-primary fw-normal" id="aceleracao_centripeta_display">0 m/s²</h4>
                            <p class="fs-11 fw-semi-bold text-500 mb-0">Aceleração Centrípeta (a<sub>c</sub>)</p>
                        </div>
                        <div class="col">
                            ...
                            <div class="row ">
                                <div class="col">
                                    <div class="mb-3"><label class="form-label" for="exampleFormControlInput1">
                                            Velocidade Inicial (m/s):</label><input id="velocidade_inicial" min="1" max="100"
                                            class="form-control" type="number" placeholder="Valor" />
                                    </div>
                                </div>
                                <div class="col">
                                    <div class="mb-3"><label class="form-label" for="customRange1"> Aceleração Tangencial (m/s²):</label><input class="form-control" value="1" id="aceleracao_tangencial"
...
function simular() {
    ...
    var velocidadeInicial = parseFloat(document.getElementById('velocidade_inicial').value);
    var aceleracaoTangencial = parseFloat(document.getElementById('aceleracao_tangencial').value);
    var raio = parseFloat(document.getElementById('raio').value);

    var velocidadeAngularInicial = velocidadeInicial / raio;
    ...
    intervalID = setInterval(function() {
        ...
        // Calcular a velocidade angular em função do tempo
        var velocidadeAngular = velocidadeAngularInicial + (aceleracaoTangencial * tempo / raio);

        // Calcular a posição angular em função do tempo
        var posicaoAngular = velocidadeAngularInicial * tempo + 0.5 * (aceleracaoTangencial * Math.pow(tempo, 2) / raio);

        // Converter a posição angular em coordenadas cartesianas
        var x = raio * Math.cos(posicaoAngular);
        var y = raio * Math.sin(posicaoAngular);

        // Desenhar o objeto (bola vermelha)
        ctx.beginPath();
        ctx.arc(canvas.width / 2 + x, canvas.height / 2 + y, 5, 0, 2 * Math.PI);
        ctx.fillStyle = '#ff0000';
        ctx.fill();

        // Desenhar o texto da velocidade na bola vermelha
        ctx.font = '7px Arial'; // Definir uma fonte mais nítida e em negrito
ctx.fillStyle = '#ffffff'; // Cor do texto
ctx.textAlign = 'center';
ctx.fillText(velocidadeAngular.toFixed(2) + ' rad/s', canvas.width / 2 + x, canvas.height / 2 + y + 5); // Ajustar a posição vertical do texto
        ...
        var aceleracaoCentripeta = Math.pow(velocidadeAngular, 2) * raio;
        document.getElementById("aceleracao_centripeta_display").textContent = `${aceleracaoCentripeta.toFixed(2)} m/s²`;

        // Calcular o período e a frequência
        var periodo = 2 * Math.PI / velocidadeAngular;
        var frequencia = 1 / periodo;

        // Atualizar os dados de tempo e frequência exibidos
...
function reset() {
    ...
    document.getElementById("velocidade_angular_display").textContent = `0 rad/s`;
    document.getElementById("aceleracao_centripeta_display").textContent = `0 m/s²`;
    document.getElementById("periodo_display").textContent = `0 s`;
    document.getElementById("frequencia_display").textContent = `0 Hz`;
}
