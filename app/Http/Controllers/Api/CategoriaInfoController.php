...
use App\Http\Resources\CategoriaInfoResource;

class CategoriaInfoController extends Controller
{
    public function index()
    {
        try {
            $categoriaInfos = categoriaInfos()->get();
            if ($categoriaInfos->isEmpty()) {
                return response()->json(['message' => 'Nenhuma  categoria de anexo encontrado.'], 200);
            }
            // Retornar os dados com status de sucesso
             return response()->json($categoriaInfos, 200);
             $data=CategoriaInfoResource::collection($categoriaInfos);
            //  return $data;
            return $data;
        } catch (\Exception $e) {
    ...
    public function store(Request $request)
    {
        try {
            ...
            $categoriaInfo = CategoriaInfo::create([

                'vc_nome' => $request->vc_nome
            ]);

            // return new CategoriaInfosResource($anexo);
            if ($categoriaInfo) {
    ...
    {
        $categoriaInfo['data'] = CategoriaInfo::where('id', $id)->first();

        if (!$categoriaInfo) {
            return response()->json(['message' => 'CategoriaInfo Não Encontrado'], 200);
        }

        // return new UserResource($user);
        return response()->json($categoriaInfo,200);
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $registro = CategoriaInfo::find($id)->update([
                'vc_nome' => $request->vc_nome,
            ]);
            if(!$registro){
                return response()->json([
                ...
                return response()->json([
                    'status' => 200,
                    'message' => "Categoria" .$request->vc_nome." Atualizado com sucesso",
                ], 200);
    ...
    {
        try {
            $registro = CategoriaInfo::find($id);
            if ($registro) {
                $registro->delete();
