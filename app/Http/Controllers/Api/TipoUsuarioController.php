...
class TipoUsuarioController extends Controller
{
    ...
    public function cadastrar(Request $request)
    {
        try {
            ...
            $user = TipoUsuario::create([
                'nome' => $request->nome,
            ]);

            if ($user) {
                $ultimoUsuario = TipoUsuario::latest()->first();
    ...
    public function actualizar(Request $request, $id)
    {
        try {
            ...
            $registro = TipoUsuario::find($id)->update([
                "nome" => $validated["nome"],
            ]);
            ...
            if(!$registro){
                ...
                // return response()->json("Tipo de Usuario ".$request->vc_pnome." ".$request->vc_unome." Atualizado com sucesso", 200);
            }
    ...
    {
        $user['data'] = TipoUsuario::where('id', $id)->first();

        if (!$user) {
            return response()->json(['message' => 'Usuário Não Encontrado'], 200);
        }

        // return new TipoUsuarioResource($user);
